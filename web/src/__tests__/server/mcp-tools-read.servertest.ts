...
describe("MCP Read Tools", () => {
  ...
  maybeEventsTable("getObservationFilterMetadataKeys tool", () => {
    ...
    it("should expose selective scope inputs in the tool schema", () => {
      expect(
        ...
        expect.objectContaining({
          ...
          hasParentObservation: expect.any(Object),
    ...
    it("should include root observations when filtering metadata keys by hasParentObservation false", async () => {
      const { context, projectId } = await createMcpTestSetup();
      const traceId = randomUUID();
      const rootObservation = createObservationEvent({
        projectId,
        traceId,
        metadata: { rootOnly: "true" },
        parentObservationId: null,
      });
      const childObservation = createObservationEvent({
        projectId,
        traceId,
        metadata: { childOnly: "true" },
        parentObservationId: rootObservation.id,
      });

      await createEventsCh([rootObservation, childObservation]);

      const result = (await handleGetObservationFilterMetadataKeys(
        { traceId, hasParentObservation: false, limit: 100 },
        context,
      )) as { keys: Array<{ key: string; count: number }>; meta: object };

      expect(result.keys).toEqual([{ key: "rootOnly", count: 1 }]);
      expect(result.meta).toEqual({});
    });
