...
const GetObservationFilterMetadataKeysInputSchema = z.object({
  observationIds: z
    .array(z.string())
    .max(MAX_OBSERVATION_IDS)
    .optional()
    .describe(
      `Observation IDs to inspect. Up to ${MAX_OBSERVATION_IDS} IDs are accepted and satisfy the selective-scope requirement.`,
    ),
  traceId: z
    .string()
    .optional()
  ...
  hasParentObservation: z
    .boolean()
    .optional()
    .describe("Optionally restrict discovery to child or root observations."),
...
export const [
  ...
  baseSchema: GetObservationFilterMetadataKeysInputSchema,
    ...
    return await runMcpTool({
      ...
      fn: async () => {
        ...
        if (input.fromStartTime && input.toStartTime) {
          ...
          const rangeMs = toStartTime.getTime() - fromStartTime.getTime();

          if (rangeMs <= 0) {
            throw new InvalidRequestError(
              "fromStartTime must be before toStartTime",
            );
          }
